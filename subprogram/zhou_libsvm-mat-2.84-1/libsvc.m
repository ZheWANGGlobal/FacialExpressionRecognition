function model=libsvc(x,y,ker,C,Tol,ShrinkOn,ProbabilityOn,CrossValidationOn,Weight)
%Solve support vector machine for classification by using lib svm software
%usage:
%model=libsvc(x,y,ker,C,Tol,ShrinkOn,ProbabilityOn,CrossValidationOn,Weight);
...
if nargin<4 | nargin>9
    ...
    if nargin<6, ShrinkOn=0;,end
    if nargin<5, Tol=1e-5;,end
    global p1 p2 p3;
    [numsam,dim]=size(x);
    LibSVMTrainOptions=[];
    LibSVMTrainOptions=[LibSVMTrainOptions,' -s 0 '];
    switch lower(ker)
        case 'linear'
