function model=libsvr(x,y,ker,C,e,Tol,ShrinkOn,ProbabilityOn,CrossValidationOn)
...
%x             --      Training examples or precomputed kernel
...
if nargin<4 | nargin>9
    ...
    if nargin<9, CrossValidationOn=0;,end
    if nargin<8, ProbabilityOn=0;,end
    if nargin<7, ShrinkOn=1;,end
    if nargin<6, Tol=1e-5;,end
    if nargin<5, e=1e-1;,end
    global p1 p2;
    [numsam,dim]=size(x);
    LibSVMTrainOptions=[];
    ...
    switch lower(ker)
        ...
        case 'precomputed'
            LibSVMTrainOptions=[LibSVMTrainOptions,' -t 3 '];
        otherwise
            LibSVMTrainOptions=[LibSVMTrainOptions,' -t 2 -g ',num2str(p1),' '];
    end
    LibSVMTrainOptions=[LibSVMTrainOptions,' -c ', num2str(C),' '];
    LibSVMTrainOptions=[LibSVMTrainOptions,' -p ', num2str(e),' '];
