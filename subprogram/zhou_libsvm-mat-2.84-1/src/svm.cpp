...
static void solve_guest_svm(
			const svm_problem *prob, const svm_parameter* param,
			double *alpha, Solver::SolutionInfo* si, double Cp, double Cn)
{
	int l = prob->l;
	double *cobj = new double[l];
	schar *y = new schar[l];
	...
	for(i=0;i<l;i++)
	{
		alpha[i] = prob->alpha0[i];
		cobj[i] = prob->cobj[i];
		if(prob->Aeq[i] > 0) y[i] = +1; else y[i]=-1;
	}

	Solver s;
	s.Solve(l, ONE_CLASS_Q(*prob,*param), cobj, y,
		alpha, Cp, Cn, param->eps, si, param->shrinking);
	...
	double Cp, double Cn)
{
	...
	switch(param->svm_type)
	{
		...
		case NU_SVR:
			...
			break;
		case GUEST:
			solve_guest_svm(prob,param,alpha,&si,Cp,Cn);
			break;
	}
...
const char *svm_check_parameter(const svm_problem *prob, const svm_parameter *param)
{
	...
	if(param->probability == 1 &&
	   (svm_type == ONE_CLASS || svm_type == GUEST))
		return "one-class SVM or Guest model, probability output not supported yet";
